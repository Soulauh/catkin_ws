...
//start record
	...
	while((record_state != MSP_AUDIO_SAMPLE_LAST)&&ros::ok())
	{
...
{
	ros::init(argc, argv, "rob_awaken");
	ros::NodeHandle n;
	ros::Publisher wakeup_pub = n.advertise<std_msgs::String>("rob_awaken_wakeup", 10);
	ros::Rate loop_rate(10);

	int         ret       = MSP_SUCCESS;
	const char *lgi_param = "appid = 6395c5d6,work_dir = .";
	const char *ssb_param = "ivw_threshold=0:1450,sst=wakeup,ivw_res_path =fo|/home/soulauh/catkin_ws/src/ros_voice/bin/msc/res/ivw/wakeupresource.jet";
	...
	{	
		...
		{	
			wakeupflag = false;
			ROS_INFO("I am wakeup");
			std_msgs::String a_msg;
			std::stringstream wakeup;
			wakeup << "wakeup";
			a_msg.data = wakeup.str();
			wakeup_pub.publish(a_msg);
		}
		ros::spinOnce();
	}
